---
sidebar_label: Add a funding program
sidebar_position: 9
title: Add or update a funding program
description: How to add a grant, accelerator, fund, or other program to the grants and funding page at /grants-funding, and how to keep a card accurate.
---

The [grants and funding page](/grants-funding) lists ways to get funded to build on Cardano. Each card comes from one program entry in `src/data/funding.js`. This page explains the fields and the rules for a card.
...
- **Facts, not promotion.** Describe what the program funds, who runs it, and how it works. No superlatives, no claims that cannot be checked on the source.
...
| `logoDark` | no | Alternative mark for dark mode, for single-color SVGs. |
...
| `runBy` | yes | Who runs it, as a name, for example "the Cardano Foundation". Anything longer belongs in the description. |
| `link` | no | `{ href, label? }` for the primary call to action. Leave it out when there is no official page yet. The button says "Program page" unless `label` says otherwise, for example "Open GovTool". External links open in a new tab. |
...
1. Add an entry to `getFundingPrograms()` in `src/data/funding.js`, following the fields above. Use `translate()` with ids prefixed `funding.program.<key>.`.
...
Change the fields that changed and mention the source in the pull request. A program that has shut down is removed.
