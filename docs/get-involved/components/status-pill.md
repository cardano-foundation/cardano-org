---
title: Status Pill
description: Small color-coded label for a lifecycle or availability state using the StatusPill component.
---
...
| `tone` | `'info' \| 'success' \| 'warning' \| 'neutral'` | `'neutral'` | Color of the pill. Unknown values fall back to neutral. |
...
The tints are mixed from the tokens with `color-mix()`, so the pill follows the theme in light and dark mode without extra colors.
...
<div style={{display: 'flex', flexWrap: 'wrap', gap: '0.5rem'}}>
  ...
  <StatusPill tone="success" label="Live on mainnet" />
  <StatusPill tone="warning" label="Open until Sep 30" />
