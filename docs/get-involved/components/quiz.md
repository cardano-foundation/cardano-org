...
Passing an `onRecord` function switches `Quiz` into hub mode: a tier badge (bronze, silver, or gold), an emoji result grid, a [share button](./quiz-share.md), and an optional academy call-to-action all appear on the results screen.
