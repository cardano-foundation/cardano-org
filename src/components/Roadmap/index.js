...
// One card shape for every upgrade, being built or enacted: a status pill
// where it says something, when, the name, the era, one sentence, one link.
function TimelineCard({ item }) {
  const external = /^https?:/.test(item.link.href);
  return (
    <article id={item.anchor} className={styles.card}>
